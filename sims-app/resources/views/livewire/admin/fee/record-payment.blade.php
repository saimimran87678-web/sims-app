<div>
                        ...
                        @if(!empty(trim($search)) || $filter_class)
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @forelse($studentsList as $st)
                                    <div class="bg-gray-50 dark:bg-gray-900/50 hover:bg-emerald-50/55 dark:hover:bg-emerald-950/20 p-5 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-emerald-200 dark:hover:border-emerald-800 transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group">
                                        <div class="space-y-1">
                                        ...
                                        <div class="flex gap-2 items-center w-full sm:w-auto justify-end">
                                            <a href="{{ route('admin.fee.ledger', $st->id) }}" 
                                               class="flex-1 sm:flex-none justify-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-blue-500 hover:bg-blue-500 hover:text-white dark:hover:bg-blue-600 rounded-xl font-bold text-sm transition-all shadow-sm flex items-center gap-1.5"
                                               title="View Fee Ledger">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                                Ledger
                                            </a>
                                            <button wire:click="selectStudent({{ $st->id }})" 
                                                    class="flex-1 sm:flex-none justify-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-emerald-500 hover:bg-emerald-500 hover:text-white dark:hover:bg-emerald-600 rounded-xl font-bold text-sm transition-all shadow-sm">
                                                Select
                                            </button>
                                        </div>
                                    </div>
            ...
            @else
                ...
                <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900/50 p-6 md:p-8 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        <button wire:click="resetStudentSelection" class="w-full md:w-auto justify-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 text-sm font-semibold transition-colors flex items-center gap-1.5">
                            ...
                            <table class="w-full text-left border-collapse min-w-[600px] sm:min-w-0">
                                <thead>
                                    <tr class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Billing Period</th>
                                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Total Amount</th>
                                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Balance Due</th>
                                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                                        <th class="px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Action</th>
                                    </tr>
                                </thead>
                                    ...
                                    @forelse($unpaidRecords as $rec)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            <td class="px-4 sm:px-6 py-3 sm:py-4">
                                                <div class="font-bold text-gray-900 dark:text-white text-sm sm:text-base">{{ \Carbon\Carbon::parse($rec->period . '-01')->format('F Y') }}</div>
                                                <div class="text-[10px] sm:text-[11px] text-gray-500 capitalize mt-0.5">{{ str_replace('_', ' ', $rec->cycle) }} cycle</div>
                                            </td>
                                            <td class="px-4 sm:px-6 py-3 sm:py-4 text-right font-medium text-gray-900 dark:text-white text-sm sm:text-base">Rs. {{ number_format($rec->total_amount, 2) }}</td>
                                            <td class="px-4 sm:px-6 py-3 sm:py-4 text-right font-black text-red-600 dark:text-red-400 text-sm sm:text-base">Rs. {{ number_format($rec->balance, 2) }}</td>
                                            <td class="px-4 sm:px-6 py-3 sm:py-4">
                                                @if($rec->period > now()->format('Y-m'))
                                                    <span class="inline-flex px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">Upcoming</span>
                                                @elseif($rec->status === 'partial')
                                                    <span class="inline-flex px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400">Partially Paid</span>
                                                @else
                                                    <span class="inline-flex px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400">Unpaid</span>
                                                @endif
                                            </td>
                                            <td class="px-4 sm:px-6 py-3 sm:py-4 text-right">
                                                <button type="button" wire:click="selectRecord({{ $rec->id }})" class="px-3 py-1.5 sm:px-4 sm:py-2 text-white rounded-xl font-bold text-xs sm:text-sm transition-colors shadow-sm" style="background-color: #059669; color: #ffffff;">
                                                    Collect Payment
                                                </button>
                                            </td>
                                        </tr>
    ...
    @if($isOpen && $record)
    @teleport('body')
        <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-3 sm:p-4">
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 dark:border-gray-755 transform scale-100 transition-all duration-300 flex flex-col max-h-[92vh]">
                <!-- Modal Header -->
                <div class="px-5 sm:px-8 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50 shrink-0">
                ...
                <form wire:submit.prevent="storePayment" class="flex flex-col flex-1 min-h-0">
                    <div class="p-4 sm:p-6 md:p-8 overflow-y-auto flex-1 space-y-4">
                        <!-- Summary Card -->
                        <div class="bg-gradient-to-br from-emerald-50/70 to-teal-50/30 dark:from-emerald-950/20 dark:to-teal-950/5 rounded-2xl p-4 sm:p-5 border border-emerald-100/50 dark:border-emerald-900/35 text-xs sm:text-sm shadow-sm">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-emerald-800 dark:text-emerald-400 font-semibold">Student:</span>
                                <span class="font-extrabold text-emerald-950 dark:text-white text-sm sm:text-base">{{ $record->student->name }}</span>
                            </div>
                            <div class="flex justify-between items-center mb-2">
                            ...
                            <div class="flex justify-between items-center text-sm sm:text-base border-t border-emerald-200/50 dark:border-emerald-900/50 pt-2.5 mt-2.5">
                                <span class="text-emerald-800 dark:text-emerald-400 font-bold">Pending Balance:</span>
                                <span class="font-black text-red-600 dark:text-red-400 text-base sm:text-lg">Rs. {{ number_format($record->balance, 2) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <input type="date" wire:model="payment_date" class="w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm focus:ring-emerald-500 focus:border-emerald-500 font-medium">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" max="{{ $record->balance }}" wire:model="amount" class="w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 font-bold focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                                @error('amount') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>

                        <!-- Pay Head Breakdown -->
                        <div class="bg-gray-50/50 dark:bg-gray-900/20 p-3 sm:p-4 rounded-2xl border border-gray-100 dark:border-gray-700/60">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Pay Head Breakdown</label>
                            <div class="overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0">
                                <table class="w-full text-left border-collapse text-[11px] sm:text-xs min-w-[390px]">
                                    <thead>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <select wire:model="payment_method" class="w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-semibold">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="reference_number" class="w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 focus:ring-emerald-500 focus:border-emerald-500 text-sm" placeholder="e.g. Cheque / Txn ID">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="remarks" class="w-full px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 focus:ring-emerald-500 focus:border-emerald-500 text-sm" placeholder="e.g. Paid by father">
                            @error('remarks') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <!-- Modal Actions -->
                    <div class="px-5 sm:px-8 py-4 border-t border-gray-100 dark:border-gray-700 flex flex-col-reverse sm:flex-row justify-end gap-3 bg-gray-50/50 dark:bg-gray-800/50 shrink-0">
                        <button type="button" wire:click="close" class="w-full sm:w-auto px-5 py-2.5 text-gray-700 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 rounded-xl font-bold transition-all text-sm">
                            Cancel
                        </button>
                        <button type="submit" wire:loading.attr="disabled" class="w-full sm:w-auto px-5 py-2.5 text-white rounded-xl font-extrabold shadow-lg transition-all text-sm disabled:opacity-75" style="background-color: #059669; color: #ffffff;">
                            Confirm Payment
                        </button>
                    </div>
                </form>
