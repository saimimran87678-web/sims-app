<div class="space-y-6 max-w-2xl mx-auto">
    ...
    {{-- Global Update Finished Floating Toast --}}
    <div x-data="{ show: false, message: '', success: true }"
         x-on:sims-update-finished.window="message = $event.detail.message; success = $event.detail.success; show = true; setTimeout(() => { show = false; }, 9000);"
         x-show="show"
         x-cloak
         ...
         class="fixed bottom-6 right-6 z-[9999] max-w-md p-4 rounded-2xl shadow-2xl flex items-center gap-3 border text-sm font-semibold backdrop-blur-md"
         :class="success ? 'bg-emerald-600/95 text-white border-emerald-500' : 'bg-red-600/95 text-white border-red-500'"
         style="display: none;">
        <svg x-show="success" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button type="button" @click="show = false" class="p-1 rounded-lg hover:bg-black/10 transition-colors">
