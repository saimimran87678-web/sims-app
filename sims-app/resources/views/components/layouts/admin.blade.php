...
<div x-data="{ sidebarOpen: window.innerWidth >= 768 }" class="flex h-screen overflow-hidden bg-gray-50">
        ...
        >
            ...
            <nav class="flex-1 py-6 space-y-1 overflow-y-auto min-h-0">
                ...
                @can('users.manage')
                <x-nav-link :href="route('admin.users')" :active="request()->routeIs('admin.users')" color="indigo">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><polyline points="17 11 19 13 23 9"/></svg>
                ...
                <x-nav-link :href="route('admin.teacher-attendance')" :active="request()->routeIs('admin.teacher-attendance')" color="orange">
                     <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="20" y1="8" x2="20" y2="14"/><line x1="23" y1="11" x2="17" y2="11"/></svg>
                    <span class="font-medium">Teacher Attendance</span>
                </x-nav-link>

                <!-- WhatsApp Setup -->

                <x-nav-link :href="route('admin.timetable')" :active="request()->routeIs('admin.timetable')" color="blue">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const isFreshLogin = @json(session('login_success', false));
            
            if (isFreshLogin) {
                sessionStorage.setItem('tab_session_active', 'true');
            } else if (!sessionStorage.getItem('tab_session_active')) {
                // No tab session found, force logout
                // Use a form to POST logout
